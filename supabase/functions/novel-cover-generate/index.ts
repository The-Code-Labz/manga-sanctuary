...
// Novel cover art generation — fal.ai, model hardcoded to OpenAI's
...
serve(async (req) => {
  ...
  const apiKey = Deno.env.get("NOVEL_SANCTUARY_FAL_API_KEY");
  ...
  if (customPrompt) {
    ...
    prompt =
      `Professional light novel cover art illustration for a novel titled "${title}". ` +
      ...
      `high-quality published light novel / web novel cover.`;
